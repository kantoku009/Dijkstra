...
if __name__ == "__main__":
	...
	class TestCaseDijkstra(unittest.TestCase):
		...
		def test_search_root(self):
			...
			"""
			theAnswer=["Node[1 0]", "Node[0 0]", "Node[0 1]", "Node[0 2]", "Node[0 3]", "Node[1 3]", "Node[1 4]", "Node[2 4]", "Node[3 4]", "Node[3 3]"]
	...
	def test_dijkstra():
		...
		@note	以下のように定義し、スタートからゴールまでの最短経路を求める.
				...
				※ます目の境界は壁で余分に埋め尽くす. つまり6 X 7のます目が必要となる.
		...
		#ダイクストラ クラスを生成.
		theDijkstra = make_dijkstra(7, 6, theBoard)
		theDijkstra.start = theBoard[1][2]
		theDijkstra.goal = theBoard[4][4]
	...
	#for theNode in theRouteList: print theNode
